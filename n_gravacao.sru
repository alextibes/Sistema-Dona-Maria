...
forward prototypes
public function boolean of_gravar (powerobject adw_salvar[])
public function boolean of_gravar (datawindow adw_salvar)
public subroutine of_rollback (boolean ab_mostrar_msg)
public function boolean of_gravar (powerobject adw_salvar[], boolean ab_mostrar_msg)
public function boolean of_comitar (boolean ab_mostrar_msg)
public function long of_get_contador (long al_tipo)
end prototypes

public function boolean of_gravar (powerobject adw_salvar[]);Return of_Gravar(adw_Salvar, True)
end function

public function boolean of_gravar (datawindow adw_salvar);Return of_Gravar({adw_Salvar})
end function

public subroutine of_rollback (boolean ab_mostrar_msg);//Fun$$HEX2$$e700e300$$ENDHEX$$o para desfazer as altera$$HEX2$$e700f500$$ENDHEX$$es
If ab_mostrar_msg Then
	MessageBox(gs_Sistema, 'Falha ao gravar: ~r~r' + SQLCA.SQLErrText, StopSign!) //Mostra a mensagem de Erro
End If

ROLLBACK USING SQLCA;
end subroutine

public function boolean of_gravar (powerobject adw_salvar[], boolean ab_mostrar_msg);Integer li_For, li_Ret

For li_For = 1 To UPPERBOUND(adw_Salvar) //Percorre Todas as dw que existem para gravar
	
	adw_Salvar[li_For].Dynamic SetTransObject(SQLCA)
	adw_Salvar[li_For].Dynamic AcceptText()
	
	li_Ret = adw_Salvar[li_For].Dynamic Update(True, False) //Grava a dw corrente
